...
;	
	...
	if(empty($username) || empty($firstname) || empty($lastname) || empty($cid) || empty($address) || empty($email) || empty($tel) || empty($date)) {	
		...
		//updating the table
		$sql = "UPDATE users SET username=:username, firstname=:firstname, lastname=:lastname, cid=:cid, address=:address, email=:email tel=:tel, date=:date, WHERE user_id=:user_id";
